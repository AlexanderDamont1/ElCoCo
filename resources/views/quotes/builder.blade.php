...
═══════════════════════════════════════════ */
        ...
        .d-block-body input[type="number"]:focus {
            border-color: var(--accent);
            box-shadow: 0 0 0 2px rgba(55, 65, 81, .08);
        }

        .d-block-body textarea {
            width: 100%;
            padding: 0.375rem 0.625rem;
            border: 1px solid var(--gray-200);
            border-radius: var(--r-sm);
        ...
        .d-block-body textarea:focus {
            border-color: var(--accent);
            box-shadow: 0 0 0 2px rgba(55, 65, 81, .08);
        }

        .d-block-body .char-count {
            font-size: 0.625rem;
            color: var(--gray-400);
            text-align: right;
            margin-top: 0.2rem;
...
═══════════════════════════════════════════ */
        ...
        @media (max-width: 768px) {
            ...
            /* ocultamos en móvil para no saturar */
            ...
            <aside class="sidebar" :class="{ open: sidebarOpen }">
                ...
                <div class="sidebar-body">
                    ...
                    <div x-show="!isLoadingBlocks && filteredCategories.length === 0" class="empty-state">
                        ...
                        <span
                            x-text="search ? 'Sin resultados para «' + search + '»' : 'No hay servicios disponibles'"></span>
                    </div>

                    <!-- Categorías -->
                    <template x-for="cat in filteredCategories" :key="cat.id">
                        <div class="cat-section">
                            <div class="cat-header" :class="{ open: cat.expanded }"
                                ...
                                <div style="display:flex;align-items:center;gap:0.5rem">
                                    ...
                                    <svg class="cat-chevron" :class="{ open: cat.expanded }" fill="none"
                            ...
                            <div class="cat-body" x-show="cat.expanded" x-transition>
                                <template x-for="block in cat.blocks" :key="block.id">
                                    <div class="block-pill" draggable="true" @dragstart="onDragStart($event, block)"
                                        @dragend="onDragEnd($event)" @click="addBlockAt(block, null, null)"
            ...
            <div class="canvas-area">
                <div class="canvas-head">
                    ...
                    <div class="canvas-controls">
                        ...
                        <!-- Acciones -->
                        <button class="btn btn-ghost btn-icon" @click="undo" :disabled="historyIdx <= 0"
                        ...
                        <button class="btn btn-ghost btn-icon" @click="redo"
                            :disabled="historyIdx >= history.length - 1" title="Rehacer (Ctrl+Y)" aria-label="Rehacer">
                            <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M21 10h-10a8 8 0 00-8 8v2M21 10l-6 6m6-6l-6-6" />
                            </svg>
                        </button>
                        <button class="btn btn-ghost" @click="clearCanvas" :disabled="placedBlocks.length === 0"
                            aria-label="Limpiar">
                ...
                <!-- Canvas viewport -->
                <div class="canvas-viewport" id="canvas-vp" @dragover.prevent @drop.prevent="onCanvasDrop">
                    ...
                    <div class="canvas-blocks" :style="{ transform: 'scale('+zoom+')' }">
                        ...
                        <template x-for="(blk, idx) in placedBlocks" :key="blk.iid">
                            <div class="d-block" :class="{ active: blk.dragging }" :data-iid="blk.iid"
                                ...
                                <div class="d-block-body">
                                    ...
                                    <!-- Campo solo para bloques personalizados (precio 0 o sin horas definidas) -->
                                    <div class="field-row" x-show="blk.base_price === 0">
                                        <label>Describe tus requerimientos</label>
                                        <textarea x-model="blk.custom_description" maxlength="256" rows="3"
                                            placeholder="¿Qué necesitas? Cuéntanos tu proyecto…" @click.stop
                                            @mousedown.stop></textarea>
                                        <div class="char-count"
                                            :class="{ warn: (blk.custom_description || '').length > 200 }">
                                            <span x-text="(blk.custom_description || '').length"></span>/256
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="d-block-foot">
                                    <div>
                                        ...
                                        <div class="d-block-hours" x-text="(blk.hours * blk.quantity) + 'h totales'">
                                        </div>
                                    </div>
                                    <button class="d-block-dup" @click="dupBlock(blk.iid)" title="Duplicar"
                                        aria-label="Duplicar">
...
═══════════════════════════════════════════════════════ -->
    <div class="modal-backdrop" x-show="showTutorial" x-cloak @click.self="showTutorial = false">
        <div class="tutorial-sheet">
            ...
            <div class="tut-body">
                <div class="tut-video">
                    <video controls preload="metadata" poster="/images/video-preview.jpg"
                ...
                <div class="tut-steps">
                    <div class="tut-step">
                        ...
                        <div>
                            ...
                            <p class="tut-step-desc">Arrastra bloques desde el panel izquierdo — o tócalos para
                                agregarlos directamente en móvil.</p>
                        </div>
                    </div>
                    <div class="tut-step">
                        ...
                        <div>
                            ...
                            <p class="tut-step-desc">Mueve, duplica y ajusta cantidades. El total se actualiza en tiempo
                                real.</p>
                        </div>
                    </div>
                    <div class="tut-step">
                        ...
                        <div>
                            ...
                            <p class="tut-step-desc">Pulsa "Enviar Cotización", llena los datos y te contactamos en
                                menos de 24h.</p>
                        </div>
                    </div>
                </div>
...
═══════════════════════════════════════════════════════ -->
    <div class="modal-backdrop" x-show="showSubmit" x-cloak @click.self="showSubmit = false"
        @keydown.escape.window="showSubmit = false">
        <div class="modal-sheet">
            ...
            <div class="modal-body">
                ...
                <div class="field-group">
                    ...
                    <input class="field-input" type="text" x-model="form.name" placeholder="Tu nombre o empresa"
                        autocomplete="name">
                </div>
                <div class="field-row-2">
                    <div class="field-group">
                        <label class="field-label">Correo<span class="req">*</span></label>
                        <input class="field-input" type="email" x-model="form.email" placeholder="correo@ejemplo.com"
                            autocomplete="email" inputmode="email">
                    </div>
                    <div class="field-group">
                        <label class="field-label">Teléfono</label>
                        <input class="field-input" type="tel" x-model="form.phone" placeholder="+52 123 456 7890"
                            autocomplete="tel" inputmode="tel">
                    </div>
                </div>
                <div class="field-group">
                    <label class="field-label">Empresa</label>
                    <input class="field-input" type="text" x-model="form.company" placeholder="Nombre de la empresa"
                        autocomplete="organization">
                </div>
                <div class="field-group">
                    <label class="field-label">Notas adicionales</label>
                    <textarea class="field-input" x-model="form.notes"
                        placeholder="Requerimientos especiales, preguntas…" rows="3"></textarea>
                </div>
            ...
            <div class="modal-foot">
                ...
                <button class="btn btn-primary" @click="submitQuote"
        ...
        <div class="success-sheet">
            ...
            <div class="success-body">
                ...
                <div class="success-actions">
                    ...
                    <button class="btn btn-ghost" @click="showSuccess = false; placedBlocks = []">Nueva
                        cotización</button>
                </div>
            </div>
        </div>
